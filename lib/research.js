...
// Claude-only fallback (no live web research) — still personalized to profile+JD.
async function claudeFallback(profile, job, discipline) {
  const template = () => ({
    ...
    ideas: (profile.strengths || []).slice(0, 3).map((s, i) => ({
      ...
      detail: `I'd put my ${(profile.topSkills || [])[i] || "experience"} to work for ${job.company} — ${String(s).replace(/^Hands-on depth in /i, "hands-on depth in ")}.`,
    })),
...
export async function getRoleResearch(profile, job) {
  ...
  if (PARALLEL_KEY) {
    try {
      ...
      const r = await runParallelResearch(JSON.stringify(input));
      return { discipline, ...r };
    } catch (e) {
      console.warn("getRoleResearch: Parallel failed, using Claude fallback:", e.message);
    }
  }
  const fb = await claudeFallback(profile, job, discipline);
  return { discipline, ...fb };
}
