// Core hunt engine — shared by /api/outreach (manual single hunt) and
// /api/autopilot (tiered batch). A "full hunt" = contacts + pitch page +
// cadence (+ video when enabled) and costs 1 credit.
...
export async function runFullHunt({ userId, state, job, base }) {
  ...
  // means 3 extra Claude calls (draft + 2 fact-checks) that would blow the hunt's
  ...
  return updateUserState(userId, (s) => {
    ...
    s.credits.balance -= 1;
    s.credits.used += 1;
    return { pitchUrl, contacts: contacts.length, cadenceSteps: cadence.steps.length, creditsLeft: s.credits.balance };
  });
}
