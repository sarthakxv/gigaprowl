...
export function scoreJob(profile, job) {
  const skills = asArr(profile?.skills).map(String);
  const topSkills = asArr(profile?.topSkills).map(String);
  const roles = asArr(profile?.roles).map(String);
  const title = String(job?.title || "");
  const text = (title + " " + (job?.description || "") + " " + asArr(job?.tags).join(" ")).toLowerCase();

  // Skills overlap (0-50)
  const hits = skills.filter((s) => s && text.includes(s.toLowerCase()));
  const topHits = topSkills.filter((s) => s && text.includes(s.toLowerCase()));
  const skillScore = Math.min(50, hits.length * 5 + topHits.length * 5);

  // Role title relevance (0-20)
  const roleScore = roles.some((r) =>
    title.toLowerCase().includes(r.toLowerCase().replace(/senior |staff |lead /g, ""))
  ) ? 20 : roles.some((r) => r.toLowerCase().split(" ").some((w) => w.length > 3 && title.toLowerCase().includes(w))) ? 10 : 0;

  // Seniority alignment (0-15)
  const diff = Math.abs((SENIORITY_RANK[profile?.seniority] ?? 1) - jobSeniority(title));
