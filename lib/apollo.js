...
// Same search path as findContacts, but never returns Alex/Jordan demo rows.
// Used by the /test/contacts probe. Hunt still wraps this with demoContacts.
export async function probeContacts(job) {
  ...
  const debug = {
    ...
    orgFilter: job.companyDomain ? "q_organization_domains_list" : job.company ? "q_keywords" : null,
    orgValue: job.companyDomain
      ? job.companyDomain.replace(/^www\./, "")
      : (job.company || null),
    pass1: { ok: false, status: null, count: 0, people: [] },
    pass2: { used: false, ok: false, status: null, count: 0, matched: 0, dropped: [], people: [] },
    enrichment: { attempted: 0, kept: 0, failures: [] },
    reason: null,
    errorNote: null,
  };
  ...
  if (!pass1.ok) {
    debug.pass1.status = pass1.res.status;
    debug.errorNote = await apolloErrorNote(pass1.res);
    debug.reason = "apollo_http_error";
    return { contacts: [], debug };
  }
  debug.pass1.ok = true;
  debug.pass1.count = pass1.people.length;
  debug.pass1.people = pass1.people.map(slimHit);

  let hits = pass1.people;

  if (!hits.length) {
    debug.pass2.used = true;
    const pass2 = await searchPeople(key, job, { seniorities: SENIORITY_WIDEN, perPage: 10 });
    if (!pass2.ok) {
      debug.pass2.status = pass2.res.status;
      debug.reason = "apollo_http_error";
      return { contacts: [], debug };
    }
    debug.pass2.ok = true;
    debug.pass2.count = pass2.people.length;
    debug.pass2.people = pass2.people.map(slimHit);
    const dropped = [];
    const matched = [];
    for (const hit of pass2.people) {
      if (sameCompany(hit, job)) matched.push(hit);
      else dropped.push(slimHit(hit));
    }
    debug.pass2.matched = matched.length;
    debug.pass2.dropped = dropped;
    if (matched.length) hits = rankByRelevance(matched, titles, job);
  }

  if (!hits.length) {
    debug.reason = debug.pass2.used
      ? (debug.pass2.count ? "pass2_filtered" : "pass1_empty")
      : "pass1_empty";
    return { contacts: [], debug };
  }
  ...
  for (const hit of hits.slice(0, 3)) {
    debug.enrichment.attempted += 1;
    const full = hit.id ? await enrichPerson(key, hit.id) : null;
    if (!full?.first_name || !full?.last_name) {
      debug.enrichment.failures.push({
        ...slimHit(hit),
        why: !hit.id ? "no_id" : !full ? "match_failed" : "missing_name",
      });
      continue;
    }
    enriched.push(mapPerson(full, job));
    debug.enrichment.kept += 1;
  }

  if (!enriched.length) {
    debug.reason = "enrichment_empty";
    return { contacts: [], debug };
  }

  return { contacts: await enrichEmails(enriched, job), debug };
}
...
export async function findContacts(job) {
  ...
  if (debug.reason === "missing_key") {
    return enrichEmails(demoContacts(job, titles, "demo — add APOLLO_API_KEY for real contacts"), job);
  }
  if (debug.reason === "enrichment_empty") {
    return enrichEmails(demoContacts(job, titles, "demo — Apollo found contacts but enrichment returned no usable profiles"), job);
  }
