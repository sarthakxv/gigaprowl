...
async function safeFetch(url, opts = {}) {
  try {
    const r = await fetch(url, {
      ...opts,
      headers: { "User-Agent": "Mozilla/5.0 (compatible; ProwlBot/1.0)", Accept: "application/json", ...(opts.headers || {}) },
      next: { revalidate: 0 },
    });
    if (!r.ok) return null;
    return await r.json();
...
// Best-effort company domain from a name (feeds the pitch video's website slide).
function domainGuess(company) {
  const slug = (company || "").toLowerCase().replace(/\b(inc|llc|ltd|corp|co|gmbh|ai)\b/g, "").replace(/[^a-z0-9]/g, "");
...
export async function fetchRemotive(query = "software") {
  ...
  return (data?.jobs || []).map((j) => ({
    ...
    title: j.title, company: j.company_name, companyDomain: domainGuess(j.company_name),
...
export async function fetchRemoteOK() {
  ...
  return data.filter((j) => j && j.id && j.position).map((j) => ({
    ...
    title: j.position, company: j.company, companyDomain: domainGuess(j.company),
  ...
  for (let p = 1; p <= pages; p++) {
    ...
    for (const j of data?.results || []) {
      out.push({
        ...
        title: j.name, company: j.company?.name, companyDomain: domainGuess(j.company?.name),
  ...
  const tasks = ADZUNA_COUNTRIES.map((c) =>
    safeFetch(`https://api.adzuna.com/v1/api/jobs/${c}/search/1?app_id=${ADZUNA_APP_ID}&app_key=${ADZUNA_APP_KEY}&results_per_page=50&what=${encodeURIComponent(query)}&content-type=application/json`)
      .then((d) => (d?.results || []).map((j) => ({
        ...
        title: j.title, company: j.company?.display_name || "Unknown", companyDomain: domainGuess(j.company?.display_name),
...
export async function fetchGreenhouse(board) {
  ...
  return (data?.jobs || []).map((j) => ({
    ...
    title: j.title, company: co, companyDomain: domainGuess(co),
...
export async function fetchLever(board) {
  ...
  return (Array.isArray(data) ? data : []).map((j) => ({
    ...
    title: j.text, company: co, companyDomain: domainGuess(co),
...
export async function fetchSmartRecruiters(board) {
  ...
  return (data?.content || []).map((j) => ({
    ...
    title: j.name, company: j.company?.name || co, companyDomain: domainGuess(j.company?.name || co),
