...
const FACT_RULES = `
Use ONLY facts present in the resume text. NEVER invent numbers, metrics, company names, titles, projects, degrees, or outcomes. If the resume lacks specifics, write qualitatively (e.g. 'led placement operations') rather than inventing specifics ('90+ interviews') unless that exact figure appears in the resume. When in doubt, leave it out. Write like a person. No em dashes.`;
...
export async function generateSocialPosts(profile, job, pitch) {
  const fallback = {
    ...
    twitterThread: [
      ...
      `if you're hiring for ${(job.title || "this kind of role").toLowerCase()} work, or just building in the same space, my DMs are open.`,
    ],
