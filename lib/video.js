...
// All functions throw on failure — callers degrade gracefully to script-only pitches.
...
export async function cloneVoiceHeyGen(buffer, mime = "audio/mpeg", name = "prowl-voice") {
...
async function bgFromUrl(url, timeoutMs = 18000) {
  try {
    const res = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0 (compatible; ProwlBot/1.0)" }, signal: AbortSignal.timeout(timeoutMs) });
