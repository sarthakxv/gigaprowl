    ...
    <tr><td align="center">
      <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="max-width:480px;width:100%;background:#121816;border:1px solid #1e2724;border-radius:16px;overflow:hidden;">
        <tr><td style="padding:28px 32px 8px;">
          <div style="font-size:22px;font-weight:800;color:#ffffff;letter-spacing:-0.5px;">prowl<span style="color:#37E2A4;">.</span></div>
        </td></tr>
        ...
        <tr><td style="padding:20px 32px;border-top:1px solid #1e2724;color:#8A9490;font-size:12px;line-height:1.5;">
          Prowl — the smartest job hunter in the world.<br/>
...
export function verifyEmail({ name, url }) {
  const html = shell("Confirm your email", `
    <p>Hi ${name || "there"}, welcome to Prowl 👋</p>
    <p>Confirm this email address to activate outreach and start landing interviews.</p>
  ...
  const text = `Welcome to Prowl!\n\nConfirm your email to activate your account:\n${url}\n\nThis link expires in 3 days. If you didn't sign up, ignore this email.`;
  return { subject: "Confirm your email for Prowl", html, text };
}

export function resetEmail({ name, url }) {
  const html = shell("Reset your password", `
    <p>Hi ${name || "there"},</p>
    <p>We got a request to reset your Prowl password. Click below to choose a new one.</p>
  ...
  const text = `Reset your Prowl password:\n${url}\n\nThis link expires in 1 hour. If you didn't request it, ignore this email.`;
  return { subject: "Reset your Prowl password", html, text };
}
