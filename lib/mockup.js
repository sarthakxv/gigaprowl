...
export async function generateMockupImage({ brief, idea, discipline = "product", company = "the company" }) {
  ...
  try {
    ...
    const msg = await client.messages.create({
      model: MODEL,
      max_tokens: 4000,
      system: "You are a world-class product & brand designer who outputs pixel-clean SVG mockups. Output ONLY a single self-contained <svg> element — no prose, no code fences.",
      messages: [{
        role: "user",
