...
import { SESSION_COOKIE, SESSION_MAX_AGE_SEC, DEV_SESSION_SECRET, EMAIL_RE } from "@/lib/constants";
...
export function createSessionToken(userId) {
  const payload = Buffer.from(JSON.stringify({ uid: userId, exp: Date.now() + SESSION_MAX_AGE_SEC * 1000 })).toString("base64url");
...
// Bind short-lived OAuth state to the exact signed login session without
// persisting or exposing the session cookie itself.
