  ...
  return (
    <div className="min-h-screen bg-ink text-white font-body">
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <h1 className="font-display text-4xl md:text-5xl font-bold leading-tight mb-4">{content.headline}</h1>
        <p className="text-fog text-lg mb-10">{content.subhead}</p>

        <div className="bg-panel border border-edge rounded-2xl p-6 mb-10">
          <p className="text-sm text-fog uppercase tracking-widest mb-3">Personalized video pitch</p>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-10">
          {content.valueProps.map((v, i) => (
            <div key={i} className="bg-panel border border-edge rounded-2xl p-5">
              ...
              <p className="text-fog text-sm leading-relaxed">{v.detail}</p>
            </div>
          ))}
        </div>

        <div className="bg-panel border border-edge rounded-2xl p-6 mb-10">
          <p className="text-sm text-fog uppercase tracking-widest mb-3">Why {job.company}, specifically</p>
          <p className="leading-relaxed text-white/90">{content.companyAngle}</p>
        </div>

        <div className="text-center py-8">
          <p className="text-fog mb-4">{prof.name} · {prof.title} · {prof.yearsExperience} yrs</p>
