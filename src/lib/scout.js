// Gigaprowl Scout. Paste a LinkedIn URL, infer a profile, pull jobs from the
// last 48 hours, score them, and show the top few. Email is optional lead
// capture. It does not gate the report.
//
// Degrades without keys: no AI key uses a heuristic demo profile; no live jobs
// uses the seed pool so the page still renders.
...
// Caches successful reads briefly to avoid repeat LinkedIn views.
export async function inferProfile(linkedinUrl, { refresh = false } = {}) {
  const handle = handleFromUrl(linkedinUrl) || "this candidate";
  if (unipileOn()) {
    const cacheKey = kvKeys.scoutProfile(identifierFromUrl(linkedinUrl) || handle);
    try {
      if (!refresh) {
